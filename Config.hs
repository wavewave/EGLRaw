...
config bInfo = do 
  (excode, out, err) <- readProcessWithExitCode "pkg-config" ["--libs", "egl"] ""
  liboptset' <- case excode of 
                  ExitSuccess -> do  
                    return . Just .  mkLibraryOptionSet . words $ out
                  _ -> do 
                    error "pkg-config failure"
  (excode2,out2,err2) <- readProcessWithExitCode "pkg-config" ["--cflags", "egl"] ""
  incdir' <- case excode2 of 
               ExitSuccess -> do  
                 return . Just . drop 2 . head . words $ out2
               _ -> do 
                 error "pkg-config --cflags failure" 
